...
export function RequestModal({ request, mode, onClose }: RequestModalProps) {
  ...
  const peerId = request
    ? mode === "sent"
      ...
      : request.senderId
    : null;
  const { data: user, isLoading: userLoading } = useUser(peerId);

  if (!request) return null;

  const displayUser = user ?? { id: peerId!, username: "" };
  const isLoading =
    submitting || respond.isPending || cancel.isPending || userLoading;
